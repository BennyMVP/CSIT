%% SEGMENTAZIONE IBRIDA POLMONI MIGLIORATA CT
...
colormap gray;
colorbar;
title('Immagine TAC originale normalizzata');

%% Pre-processing

sigma = 0.8;
im_f = imgaussfilt(im, sigma);
...
colormap gray;
colorbar;
title('Immagine dopo pre-processing');
...
x0 = max(1, min(x0, size(im_f,2)));
y0 = max(1, min(y0, size(im_f,1)));
...
r2 = min(size(im_f,1), y0 + raggio_roi);
c1 = max(1, x0 - raggio_roi);
c2 = min(size(im_f,2), x0 + raggio_roi);

roi = im_f(r1:r2, c1:c2);
...
colormap gray;
colorbar;
title('ROI locale intorno al punto scelto');
...
lista_tol = [0.03 0.04 0.05 0.06 0.08 0.10 0.12 0.15];

area_min = 20;
area_max = round(0.45 * numel(roi));
...
mask_rg_best = imclose(mask_rg_best, strel('disk', 1));
...
mask_init = imclose(mask_init, strel('disk', 1));
...
smoothFactor_polmoni = 1.0;
contractionBias_polmoni = 0.2;

tic;

mask_ac = activecontour(roi, mask_init, num_iter, 'Chan-Vese', ...
    'SmoothFactor', smoothFactor_polmoni, ...
    'ContractionBias', contractionBias_polmoni);
...
mask_ac = imclose(mask_ac, strel('disk', 1));
...
sgtitle('Evoluzione della segmentazione ibrida migliorata CT');
...
if isfile(gt_file)
    ...
    figure;
    imshow(im, []);
    title('Disegna contorno polmone / regione da segmentare - Ground Truth');
...
x2_bb = min(size(im,2), ceil(bb(1) + bb(3) + margin));
y2_bb = min(size(im,1), ceil(bb(2) + bb(4) + margin));

roi_bb = false(size(im));
...
fprintf('\n================================================================================\n');
fprintf('  METRICHE - Segmentazione Ibrida CT - bounding box GT, margine %d px\n', margin);
fprintf('================================================================================\n');

fprintf('%-30s | %-7s | %-7s | %-8s | %-8s | %-12s | %-8s\n', ...
    'Metodo', 'Dice', 'IoU', 'HD', 'HD95', 'Prec/Rec', 'Tempo(s)');

fprintf('%s\n', repmat('-', 1, 110));

fprintf('%-30s | %.4f  | %.4f  | %8.2f | %8.2f | %.3f/%.3f | %8.2f\n', ...
    'Ibrido CT', dice_val, iou_val, hd_val, hd95_val, ...
...
overlay = cat(3, ones(size(im)), zeros(size(im)), zeros(size(im)));
h = imagesc(overlay);

alpha(h, 0.45 * double(mask_finale));

title(sprintf('Segmentazione ibrida migliorata polmoni - Dice = %.3f', dice_val));
